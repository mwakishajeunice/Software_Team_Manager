...
<div class="container-fluid">
	<form action="" id="manage-task">
		...
		<div class="form-group">
			...
			<?php 
                            $users = $conn->query("SELECT *, concat(firstname,' ',lastname) as name FROM users where type = 3");
                            while($user = mysqli_fetch_assoc($users)){
                              
			    ?>
                        	<option value="<?php echo $user['id'] ?>" <?php echo isset($task) && $user_id == $user['id'] ? 'selected' : '' ?>><?php echo $user['name'] ?></option>
			<?php }?>
			</select>
		</div>
	</form>
